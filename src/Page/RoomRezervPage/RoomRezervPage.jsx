...
import "./RoomRezervPage.scss";

const ROOMS_API = "https://api.albanproject.az/api/rooms/";
const RES_API = "https://api.albanproject.az/api/room-reservations/";
...
function RoomRezervPage() {
  ...
  // 1 saatlıq bloklama üçün xəbərdarlıq
  ...
  const loadRooms = async () => {
    ...
    setLoading(true);
    try {
      const rData = await fetchJson(ROOMS_API);
      setOtaqlar(Array.isArray(rData) ? rData : []);
    } catch (e) {
  ...
  const selectedRoom = useMemo(
    () => otaqlar.find((r) => r.id === selectedRoomId) || null,
    [otaqlar, selectedRoomId]
  );

  const activeReservation = useMemo(() => {
    if (!selectedRoom) return null;
    const list = Array.isArray(selectedRoom.reservations)
  ...
  /* Click handlers */
  const handleRoomClick = (otaq) => {
    setSelectedRoomId(otaq.id);
    setIsPopupOpen(true);
  };
  const closePopup = () => {
  ...
  }, [selectedRoom, reservationDate, reservationTime, duration]);
  ...
  */
  useEffect(() => {
    ...
    if (!selectedRoom || !reservationDate || !reservationTime) return;

    const st = selectedRoom.room_status;
  ...
  }, [selectedRoom, reservationDate, reservationTime]);

  /* Payload & Submit */
  const buildPayload = () => {
    if (!selectedRoomId) throw new Error("Otaq seçilməyib.");
    ...
    const chairNum =
      Number(
        otaqlar.find((r) => r.id === selectedRoomId)?.room_chair_number
      ) || 5;

    return {
      room: selectedRoomId,
      room_chair_number: chairNum,
      date: formattedDate,
  ...
  const handleReservationSubmit = async (e) => {
    ...
    try {
      ...
      setOtaqlar((prev) =>
        prev.map((r) => {
          if (r.id !== saved.room) return r;
          const arr = Array.isArray(r.reservations) ? [...r.reservations] : [];
  ...
  if (error) {
    return (
      <div className="rezerv-page error">
        ...
        <p>{String(error)}</p>
        <button onClick={loadRooms}>Yenidən yüklə</button>
      </div>
    );
  }
  ...
  return (
    <div className="rezerv-page">
      <div className="container">
        <header>
          <h1>Kabinet Rezervasiya Sistemi</h1>
        </header>
        ...
        <div className="otaqlar-grid">
          {otaqlar.map((otaq) => {
            const list = Array.isArray(otaq.reservations)
              ? otaq.reservations
              : [];
            const hasFuture = list.length > 0;
            const uiClass = statusToClass(otaq.room_status, hasFuture);

            return (
              <div
                key={otaq.id}
                className={`otaq ${uiClass}`}
                onClick={() => handleRoomClick(otaq)}
              >
                <div className="otaq-number">Otaq {otaq.room_num ?? otaq.id}</div>
                <div className="otaq-chairs">{otaq.room_chair_number ?? 5} stul</div>
              </div>
            );
          })}
        </div>
      ...
      {isPopupOpen && selectedRoom && (
        <div className="popup-overlay" onClick={closePopup}>
          <div className="popup-content" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="popup-title">Otaq Məlumatları</h2>

            <div className="popup-info">
              <div className="info-item">
                <span className="info-label">Otaq Nömrəsi:</span>
                <span className="info-value">
                  {selectedRoom.room_num ?? selectedRoom.id}
                </span>
              </div>
              <div className="info-item">
                <span className="info-label">Stul Sayı:</span>
                <span className="info-value">
                  {selectedRoom.room_chair_number ?? 5}
                </span>
              </div>
              <div className="info-item">
                ...
                <span
                  className={`info-value status-text ${
                    selectedRoom?.room_status === "empty"
                      ? "empty"
                      : selectedRoom?.room_status === "reserved"
                      ? "reserved"
                      : "occupied"
                  }`}
                >
      ...
      {/* Rezervasiya Formu */}
      {isReservationFormOpen && selectedRoomId && (
            ...
            >
              <div className="form-row">
                <div className="form-group">
                  ...
                  <TextField
                    value={
                      otaqlar.find((r) => r.id === selectedRoomId)?.room_num ??
                      selectedRoomId
                    }
                    disabled
                    fullWidth
